...
#if defined(NDEBUG)
# error "Blooddonation cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x93a2aa3fd2b00264b89a7ae6383b959ee54402dca431f34350b2a0f8fb71d015");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Blooddonation: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Blooddonation Signed Message:\n";
...
{
    // Blooddonation: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1){
 nSubsidy = 1050000000 * COIN;
}
	if ( nHeight > 1 && nHeight < 12200)
	{
  nSubsidy = 450 * COIN;
	}
else if (nHeight > 10000){
 for (int Loop = 0; Loop < nHeight/99; Loop++)
	{
  nSubsidy = (nSubsidy/100)*99;
	}
}
...
static const int64 nTargetTimespan = 2 * 60 * 60; // Blooddonation: 1 hours
static const int64 nTargetSpacing = 5 * 60; // Blooddonation: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Blooddonation: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Blooddonation: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x93a2aa3fd2b00264b89a7ae6383b959ee54402dca431f34350b2a0f8fb71d015");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "For blood donor. 08.06.17";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042184546fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1496945415;
        ...
        assert(block.hashMerkleRoot == uint256("0xdd0e51adbfd13a2be9af60b6d83b6af74ee0f1e135e8ad687a3b53885e17326c"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xa0, 0xb6, 0xdb }; // Blooddonation: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlooddonationMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BlooddonationMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BlooddonationMiner(CWallet *pwallet)
{
    printf("BlooddonationMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blooddonation-miner");
    ...
    try { loop {
        ...
        printf("Running BlooddonationMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BlooddonationMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlooddonationMiner, pwallet));
}
